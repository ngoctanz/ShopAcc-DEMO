...
/**
 * Minimal user data to store in localStorage
 * Only keep fields needed for UI display (header, menu)
 * Full user data is available in memory state after refresh token
 */
type CachedUser = Pick<User, 'name' | 'role' | 'balance' | 'avatar'>;
 ...
 */
async function callRefreshTokenAPI(): Promise<{ accessToken: string; user: User } | null> {
  try {
    const response = await fetch(`${APP_CONFIG.API.BASE_URL}${API_ROUTES.AUTH.REFRESH}`, {
      method: 'POST',
 ...
 */
export async function refreshTokenSingleton(): Promise<{ accessToken: string; user: User } | null> {
  if (refreshPromise) {
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const refreshAccessToken = useCallback(
    async (shouldClearOnFail = true): Promise<string | null> => {
      ...
      if (result) {
        ...
        setUser(result.user);
        // Only persist minimal user data to localStorage
        storage.set(APP_CONFIG.STORAGE_KEYS.USER, filterUserForStorage(result.user));
        return result.accessToken;
      }
      // Refresh failed - only clear auth if explicitly requested
  ...
  const refreshUser = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${APP_CONFIG.API.BASE_URL}${API_ROUTES.AUTH.ME}`, {
        headers: {
      ...
      channel.onmessage = async (event: MessageEvent<AuthMessage>) => {
        ...
        switch (message.type) {
          ...
          case 'USER_UPDATE':
            setUser(message.user);
            // Only persist minimal user data to localStorage
            storage.set(APP_CONFIG.STORAGE_KEYS.USER, filterUserForStorage(message.user));
            break;
        }
      };
